...
export default function Admin(){
    ...
    useEffect(() => {
        axios.get(`${API}/api/file/${userId}`).then(res => {
    ...
    function clickUpload(e) {
        ...
        axios.post(`${API}/api/files`,formData,{
            headers:{
    ...
    function clickDelete(fileId){
        axios.delete(`${API}/api/delete-file/${fileId}`).then(res => {
            if(res.status === 200){
                setCount(count - 1)
            }
        }).catch(error => {
        ...
        <main class="main">
            <div class="wrapper">
                ...
                <form id="videoForm" action="#" class="site-form">
                    <label>
                        <input onChange={e => setTitle(e.target.value)} type="text" id="videoInput" placeholder="video title" required />
                    </label>
                    <label class="custom-upload">
                        <span class="zmdi zmdi-upload"></span>
                        <span class="file-name">click upload a video</span>
                        <input onChange={e => setFile(e.target.files[0])} type="file" id="uploadInput" accept="video/*" />
                    </label>
                    <input onClick={clickUpload} type="submit" value="Upload" id="submitButton" />
                </form>
                <img src={imgLogin} class="signup-image" />
            </div>
            <div class="admin-wrapper">
                <ul id="videosList" class="videos-list">
                {
                    files.map(file => {
                        let yil = new Date(file.date).getFullYear()
                        let oy = new Date(file.date).getMonth()
                        let kun = new Date(file.date).getDay()
                        let soat = new Date(file.date).getHours()
                        let minut = new Date(file.date).getMinutes()
                        let sekund = new Date(file.date).getSeconds()

                        return <li class="iframe">
                        <img onClick={() => clickDelete(file.id)} className="delete" src={deleteImg} alt="error" />
                        <object data={`${API}/` + file.file} width="260px" height="200px"></object>
                        
                        <div class="iframe-footer">
                            <img src={`${API}/` + data.profilImg} alt="channel-icon" />
                            <div class="iframe-footer-text">
                                <h2 class="channel-name">{data.userName}</h2>
                                <h3 class="iframe-title">{file.title}</h3>
                                <time class="uploaded-time">{`${yil}/${oy}/${kun} | ${soat}:${minut}:${sekund}`}</time>
                                <a class="download" href={`${API}/api/files/download/${file.file}`}>
                                    <span>{Math.floor(file.size*100)}</span>
                                    <img src={download} />
                                </a>
                            </div>                  
                        </div>
